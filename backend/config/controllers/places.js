...
router.get('/:placeId', async (req, res) => {
    ...
    if (isNaN(placeId)) {
        ...
        if (!place) {
            res.status(404).json({ message: `Couldn't find place with id "${placeId}"` })
        } else {
            res.json(place)
        }
    }
...
router.post('/:placeId/comments', async (req, res) => {
    ...
    if (!place) {
        return res.status(404).json({ message: `Couldn't find place with id "${placeId}"` })
    }
...
router.delete('/:placeId/comments/:commentId', async (req, res) => {
    ...
    if (isNaN(placeId)) {
        ...
        if (!comment) {
            res.status(404).json({ message: `Couldn't find comment with id "${commentId}" for place with id "${placeId}"` })
